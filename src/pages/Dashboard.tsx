...
const EVENTS = [
  { id: '1', title: 'Tech Summit Dakar 2025', date: '15 Juin 2025', time: '09:00', location: 'Dakar Arena', category: 'Tech', status: 'upcoming', participants: 420, max: 500, revenue: '525 000', cover: 'https://images.unsplash.com/photo-1540575467063-178a50c2df87?w=80&q=80' },
  { id: '2', title: 'Festival Afrobeat Summer', date: '22 Juin 2025', time: '18:00', location: 'Plage de N\'Gor', category: 'Musique', status: 'upcoming', participants: 850, max: 1000, revenue: '12 750 000', cover: 'https://images.unsplash.com/photo-1470229722913-7c0e2dbbafd3?w=80&q=80' },
  { id: '3', title: 'Startup Weekend Africa', date: '5 Juillet 2025', time: '08:00', location: 'CTIC Dakar', category: 'Business', status: 'upcoming', participants: 120, max: 150, revenue: 'Gratuit', cover: 'https://images.unsplash.com/photo-1556761175-4b46a572b786?w=80&q=80' },
  { id: '4', title: 'Art & Culture Expo', date: '12 Juillet 2025', time: '10:00', location: 'IFAN Museum', category: 'Art', status: 'cancelled', participants: 0, max: 300, revenue: '0', cover: 'https://images.unsplash.com/photo-1578926375605-eaf7559b1458?w=80&q=80' },
]
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      <div style={{ flex: 1, overflow: 'auto' }}>
        {/* Header */}
        <header style={{ background: 'var(--bg2)', borderBottom: '1px solid var(--border)', padding: '0 32px', height: 68, display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 50, backdropFilter: 'blur(12px)' }}>
          <div>
            <h1 style={{ fontSize: '1.2rem', fontWeight: 700 }}>Vue d'ensemble</h1>
            <p style={{ fontSize: '0.78rem', color: 'var(--text3)' }}>Mercredi, 7 Mai 2025</p>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div style={{ position: 'relative' }}>
              ...
              <input
                ...
                style={{ background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 10, padding: '8px 12px 8px 36px', color: 'var(--text)', fontSize: '0.85rem', width: 200, outline: 'none' }}
            ...
            {/* Notif bell */}
            <div style={{ position: 'relative' }}>
              <button onClick={() => setShowNotifs(!showNotifs)} style={{ position: 'relative', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, width: 38, height: 38, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text2)', cursor: 'pointer', transition: 'all 0.2s' }}
              ...
              {showNotifs && (
                <div style={{ position: 'absolute', top: 48, right: 0, width: 320, background: 'var(--surface)', border: '1px solid var(--border2)', borderRadius: 16, boxShadow: '0 16px 48px rgba(0,0,0,0.5)', zIndex: 100, overflow: 'hidden' }}>
                  ...
                  {NOTIFICATIONS.map((n, i) => (
                    <div key={i} style={{ padding: '14px 20px', borderBottom: i < NOTIFICATIONS.length - 1 ? '1px solid var(--border)' : 'none', cursor: 'pointer', transition: 'background 0.2s' }}
        ...
        <main style={{ padding: 32 }}>
          ...
          <div className="fade-up" style={{ marginBottom: 32 }}>
            ...
            <p style={{ color: 'var(--text2)', fontSize: '0.92rem' }}>Tout ce qui se passe sur vos événements est résumé ici.</p>
          </div>
          ...
          <div className="fade-up-1" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 20, marginBottom: 32 }}>
            {STATS.map(({ label, value, change, positive, icon: Icon, color }) => (
              <div key={label} className="card" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                ...
                <div>
                  <div style={{ fontSize: '1.8rem', fontWeight: 800, fontFamily: 'Syne', marginBottom: 4 }}>{value}</div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    ...
                    <span style={{ fontSize: '0.78rem', color: 'var(--text3)' }}>vs mois dernier</span>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="fade-up-2 card" style={{ padding: 0, overflow: 'hidden' }}>
            <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                <Link to="/" className="btn btn-primary btn-sm"><Plus size={14} /> Nouvel événement</Link>
              </div>
            </div>
                ...
                <tbody>
                  {filtered.map((ev, _i) => (
                    <tr key={ev.id} style={{ borderTop: '1px solid var(--border)', transition: 'background 0.2s' }}
                      ...
                      {/* Date */}
                      <td style={{ padding: '16px 20px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6, color: 'var(--text2)', fontSize: '0.83rem', marginBottom: 4 }}>
                          <Calendar size={13} /> {ev.date}
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6, color: 'var(--text3)', fontSize: '0.78rem' }}>
                      ...
                      {/* Revenue */}
                      <td style={{ padding: '16px 20px' }}>
                        <span style={{ fontSize: '0.9rem', fontWeight: 600, color: ev.revenue === 'Gratuit' ? 'var(--success)' : 'var(--text)' }}>
                      ...
                      <td style={{ padding: '16px 20px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                          {[{ Icon: Eye, title: 'Voir' }, { Icon: Edit2, title: 'Modifier' }, { Icon: Trash2, title: 'Supprimer' }].map(({ Icon, title }) => (
                            <button key={title} title={title} style={{ width: 32, height: 32, borderRadius: 8, background: 'var(--surface2)', border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', color: title === 'Supprimer' ? 'var(--danger)' : 'var(--text3)', cursor: 'pointer', transition: 'all 0.2s' }}
                              onMouseEnter={e => (e.currentTarget as HTMLElement).style.background = 'var(--bg3)'}
          ...
          {/* Bottom section: Mini chart + Recent activity */}
          <div className="fade-up-3" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 24, marginTop: 24 }}>
            {/* Progression chart (visual bars) */}
            <div className="card">
              ...
              <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8, height: 120 }}>
                {[45, 72, 58, 88, 64, 95, 78].map((v, i) => (
                  <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
                    <div style={{ width: '100%', height: `${v}%`, background: i === 6 ? 'var(--accent)' : 'var(--surface2)', borderRadius: '6px 6px 0 0', transition: 'background 0.2s', cursor: 'pointer', position: 'relative' }}
                      onMouseEnter={e => { if (i !== 6) (e.currentTarget as HTMLElement).style.background = 'var(--accent2)' }}
                      onMouseLeave={e => { if (i !== 6) (e.currentTarget as HTMLElement).style.background = 'var(--surface2)' }} />
