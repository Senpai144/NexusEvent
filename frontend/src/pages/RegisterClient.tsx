...
import { Zap, Mail, Lock, Eye, EyeOff, User, ArrowRight, Globe, CheckCircle, AlertCircle, Ticket, Calendar, Shield } from 'lucide-react'
import { useAuth } from '../contexts/AuthContext'

const CLIENT_PERKS = [
  'Réservez vos événements en quelques clics',
  'Suivez toutes vos réservations en un seul endroit',
  'Recevez vos billets et confirmations par email',
  'Accédez à vos historiques et favoris'
]
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', background: 'var(--bg)' }}>
      ...
      <div className="hide-mobile" style={{ flex: 1, background: 'var(--bg2)', borderRight: '1px solid var(--border)', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', padding: 60, position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', top: '10%', right: '10%', width: 400, height: 400, background: 'radial-gradient(circle, rgba(255,92,58,0.15) 0%, transparent 70%)', borderRadius: '50%', filter: 'blur(60px)' }} />
        <div style={{ position: 'relative', maxWidth: 420 }}>
          <span className="badge badge-success" style={{ marginBottom: 20 }}>100% gratuit</span>
          ...
          <p style={{ color: 'var(--text2)', lineHeight: 1.7, marginBottom: 36 }}>
            Rejoignez NexusEvent pour réserver vos événements préférés et ne rien manquer.
          </p>
          ...
          <div style={{ marginTop: 48, padding: 20, background: 'var(--surface)', borderRadius: 'var(--radius)', border: '1px solid var(--border)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
              <Ticket size={20} color="var(--accent)" />
              <span style={{ fontWeight: 700, fontSize: '0.95rem' }}>Réservation rapide</span>
            </div>
            <p style={{ fontSize: '0.85rem', color: 'var(--text2)', lineHeight: 1.6 }}>
              Créez votre compte en 30 secondes, puis réservez n'importe quel événement en quelques étapes simples.
            </p>
          </div>
        </div>
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', padding: 'clamp(24px, 5vw, 60px)' }}>
        <div style={{ width: '100%', maxWidth: 420 }}>
          <div className="hide-desktop" style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 32, justifyContent: 'center' }}>
            ...
            <span style={{ fontFamily: 'Syne', fontWeight: 800, fontSize: '1.2rem' }}>Nexus<span style={{ color: 'var(--accent)' }}>Event</span></span>
          </div>
          ...
          <div className="fade-up" style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 28 }}>
            ...
            <span style={{ fontSize: '0.82rem', color: 'var(--text3)', marginLeft: 4 }}>
              {step === 1 ? 'Vos informations' : 'Sécurité du compte'}
            </span>
          </div>

          <h1 className="fade-up-1" style={{ fontSize: '2rem', fontWeight: 800, marginBottom: 8, letterSpacing: '-0.02em' }}>Créez votre compte</h1>
          <p className="fade-up-1" style={{ color: 'var(--text2)', marginBottom: 28, fontSize: '0.95rem' }}>
            Déjà inscrit ? <Link to="/login" style={{ color: 'var(--accent)', fontWeight: 600 }}>Connectez-vous</Link>
          </p>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
              ...
              <>
                <div className="form-group fade-up-2">
                  ...
                  <div className="input-wrap">
                    ...
                    <input type="text" className="form-input" placeholder="Votre nom complet" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} required />
                  </div>
                </div>
                <div className="form-group fade-up-3">
                  ...
                  <div className="input-wrap">
                    ...
                    <input type="email" className="form-input" placeholder="email@exemple.com" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} required />
                  </div>
                </div>
              </>
            )}
              ...
              <>
                <div className="form-group fade-up">
                  ...
                  <div className="input-wrap" style={{ position: 'relative' }}>
                    ...
                    <input type={showPwd ? 'text' : 'password'} className="form-input" placeholder="Au moins 8 caractères" value={form.password} onChange={e => setForm({ ...form, password: e.target.value })} required style={{ paddingRight: 44 }} />
            ...
            <button type="submit" className="btn btn-primary btn-lg" style={{ width: '100%', justifyContent: 'center', opacity: loading ? 0.7 : 1 }} disabled={loading}>
              {loading ? (
                <><div style={{ width: 18, height: 18, border: '2px solid rgba(255,255,255,0.3)', borderTopColor: '#fff', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} /> Création en cours...</>
              ) : step === 1 ? (
          ...
          <div style={{ marginTop: 24, display: 'flex', alignItems: 'center', gap: 8, justifyContent: 'center' }}>
            ...
            <span style={{ fontSize: '0.8rem', color: 'var(--text3)' }}>Vos données sont sécurisées et protégées</span>
          </div>
        </div>
      </div>
    </div>
