...
export default function Navbar() {
  ...
  const links = ['Événements', 'Comment ça marche', 'Contact']
    ...
    <nav style={{
      ...
      <div className="container" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 68 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }} className="hide-mobile">
          {links.map(l => (
            <a key={l} href={`#${l}`} style={{ padding: '3px 16px', borderRadius: 8, fontSize: '0.9rem', color: 'var(--text2)', transition: 'all 0.2s', fontWeight: 500 }}
            ...
            <div> 
              <Link to="/temoignages" style={{ borderRadius: 8, fontSize: '0.9rem', color: 'var(--text2)', transition: 'all 0.2s', fontWeight: 500 }}
                onMouseEnter={e => { e.currentTarget.style.color = 'var(--text)'; e.currentTarget.style.background = 'var(--surface)' }}
                onMouseLeave={e => { e.currentTarget.style.color = 'var(--text2)'; e.currentTarget.style.background = 'transparent' }}>
                Témoignages
              </Link>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }} className="hide-mobile">
          {user ? (
            <>
              ...
              <button onClick={() => logout()} className="btn btn-ghost btn-sm" style={{ gap: 6, color: 'var(--text3)' }}>
            ...
            <>
              ...
              <Link to="/register-client" className="btn btn-primary btn-sm">S'inscrire</Link>
            </>
          )}
        </div>
      ...
      {open && (
        <div style={{ background: 'var(--bg2)', borderTop: '1px solid var(--border)', padding: '16px 24px 24px', display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          {user ? (
            <>
              <Link to="/dashboard" className="btn btn-secondary" onClick={() => setOpen(false)}>Dashboard</Link>
              <button onClick={() => { logout(); setOpen(false) }} className="btn btn-ghost" style={{ justifyContent: 'center' }}>Déconnexion</button>
            </>
          ) : (
            <>
              <Link to="/login" className="btn btn-secondary" onClick={() => setOpen(false)}>Connexion</Link>
              <Link to="/register-client" className="btn btn-primary" onClick={() => setOpen(false)}>S'inscrire</Link>
            </>
          )}
        </div>
      )}
